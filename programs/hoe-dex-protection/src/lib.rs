...
/// Emitted when freeze authority warning is triggered
#[event]
...
/// Emitted when volume is decayed
#[event]
...
/// Emitted when trade execution fails
#[event]
...
/// Emitted when liquidity operation fails
#[event]
...
/// Emitted when admin operation fails
#[event]
...
/// Lightweight struct for front-end queries
#[derive(AnchorSerialize, AnchorDeserialize, Clone, Debug)]
...
mod validation {
    ...
    pub fn validate_trade_parameters(
        ...
        if pool_state.is_emergency_paused {
        ...
        if amount_in < pool_state.trade_settings.min_size {
            msg!("Trade too small: {} < {}", amount_in, pool_state.trade_settings.min_size);
            return Err(ErrorCode::TradeTooSmall.into());
        }
        if amount_in > max_trade_size {
            msg!("Trade too large: {} > {}", amount_in, max_trade_size);
            return Err(ErrorCode::TradeTooLarge.into());
        }

        // Check cooldown
        let cooldown_end = pool_state.trade_settings.last_trade_time + pool_state.trade_settings.cooldown_seconds;
        if current_time < cooldown_end {
            msg!("Cooldown active: {} seconds remaining", cooldown_end - current_time);
            return Err(ErrorCode::RateLimitExceeded.into());
        }

        // Check rate limit
        if current_time >= pool_state.rate_limit.last_reset + pool_state.rate_limit.window_seconds {
            msg!("Rate limit window expired: resetting counter");
            pool_state.reset_rate_limit(current_time)?;
        }
        if pool_state.rate_limit.count >= pool_state.rate_limit.max_calls {
            msg!("Rate limit exceeded: {} calls in window (max: {})", 
                pool_state.rate_limit.count, 
                pool_state.rate_limit.max_calls
            );
            return Err(ErrorCode::RateLimitExceeded.into());
        }

        // Check circuit breaker
        if pool_state.volume.volume_24h > pool_state.circuit_breaker.threshold {
            msg!("Circuit breaker triggered: volume {} > threshold {}", 
                pool_state.volume.volume_24h, 
                pool_state.circuit_breaker.threshold
            );
            return Err(ErrorCode::CircuitBreakerTriggered.into());
        }

        // Check daily volume
        let new_volume = pool_state.volume.volume_24h.checked_add(amount_in).ok_or_else(|| {
            msg!("Volume overflow: {} + {}", pool_state.volume.volume_24h, amount_in);
            error!(ErrorCode::Overflow)
        })?;
        if new_volume > pool_state.volume.max_daily {
            msg!("Daily volume limit exceeded: {} > {}", new_volume, pool_state.volume.max_daily);
            return Err(ErrorCode::DailyVolumeLimitExceeded.into());
        }
        ...
        if price_impact > pool_state.protection.max_price_impact_bps {
            msg!("Price impact too high: {} > {}", 
                price_impact, 
                pool_state.protection.max_price_impact_bps
            );
            return Err(ErrorCode::PriceImpactTooHigh.into());
        }
    ...
    pub fn validate_fee_parameters(
        ...
        if pool_state.fee_tiers_locked {
    ...
    pub fn validate_admin_action(
        ...
        if admin != &pool_state.admin {
            msg!("Unauthorized: expected admin {} but got {}", pool_state.admin, admin);
            return Err(ErrorCode::Unauthorized.into());
        }
        if pool_state.is_finalized {
        ...
        let cooldown_end = pool_state.last_admin_update + ADMIN_UPDATE_COOLDOWN;
        if current_time < cooldown_end {
            msg!("Admin update too frequent: {} seconds remaining", cooldown_end - current_time);
            return Err(ErrorCode::AdminUpdateTooFrequent.into());
        }
        Ok(())
    }
